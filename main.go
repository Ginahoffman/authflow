...
func sessionsHandler(c *gin.Context) {
	...
	for rows.Next() {
		var id, site, user, pass, ip, created string
		var completed int
		if err := rows.Scan(&id, &site, &user, &pass, &ip, &completed, &created); err != nil {
...
func visitsHandler(c *gin.Context) {
	...
	for rows.Next() {
		...
		var isBot int
		if err := rows.Scan(&created, &site, &ip, &isBot, &bot, &ref); err != nil {
